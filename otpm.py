...
class OTPInterceptor:
    ...
    async def start_mitmproxy(self):
        ...
        try:
            await m.run()  # Run mitmproxy in this loop
...
async def start_mitmproxy_async(interceptor):
    """Runs mitmproxy inside a properly initialized asyncio event loop."""
    await interceptor.start_mitmproxy()
...
if __name__ == "__main__":
    ...
    # Start mitmproxy in the main thread using asyncio.run
    try:
        asyncio.run(start_mitmproxy_async(interceptor))
    except KeyboardInterrupt:
